...
def run_demo(net, image_provider, height_size, cpu, track_ids):
    file_name = image_provider.file_name
    net = net.eval()
    if not cpu:
        net = net.cuda()
    ...
    for img in image_provider:
        ...
        # write keypoints to csv file
        output_path = './output/' + file_name.split('/')[-1].split('.')[0] + '_keypoints.csv'
        with open(output_path, 'a+', newline='') as csvfile:
            fieldnames = ['index', 'keypoint_id', 'keypoint_name', 'keypoint_position']
            writer = csv.DictWriter(csvfile, fieldnames=fieldnames)
            for i in range(len(keypoints_list)):
                for j in range(len(keypoints_list[i])):
                    writer.writerow({'index': i, 'keypoint_id': keypoints_list[i][j]['keypoint_id'],
                                     'keypoint_name': keypoints_list[i][j]['keypoint_name'],
                                     'keypoint_position': keypoints_list[i][j]['keypoint_position']})
